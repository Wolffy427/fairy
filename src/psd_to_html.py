...
def get_layer_html(layer, psd_info, output_dir, parent_path='', depth=0):
    """
    生成图层的HTML代码
    """
    indent = '    ' * depth
    ...
    parent_segment = layer_path.split('/')
    if len(parent_segment) > 1:
        parent_name = parent_segment[-2]
    else:
        parent_name = ''
    ...
    img_paths = [
        # 直接使用图层名称
        f"../data/image/layers/{layer_name_encoded}/{layer_name_encoded}.png",
        # 使用图层路径的最后一部分
        f"../data/image/layers/{layer_path.split('/')[-1]}/{layer_path.split('/')[-1]}.png",
        # 处理子图层路径
        f"../data/image/layers/{parent_name}/{layer_name_encoded}/{layer_name_encoded}.png",
        # 子图层特殊处理 - 使用子图层目录名称作为文件名
        f"../data/image/layers/{parent_name}/{parent_name}_sub_1/{parent_name}_sub_1.png"
    ]
    
    # 查找第一个存在的图片路径
    img_path = None
    for path in img_paths:
        full_path = os.path.join(os.path.dirname(output_dir), path)
    ...
    if is_group and 'children' in layer and layer['children']:
        ...
        for child in layer['children']:
            html += get_layer_html(child, psd_info, output_dir, layer_path, depth + 1)
        html += f"{indent}"
...
def generate_html(psd_info, output_dir, output_file):
    """
    生成完整的HTML页面
    """
    # 提取PSD基本信息
...
"""
    ...
    # 添加图层HTML
    for layer in psd_info['layers']:
        html += get_layer_html(layer, psd_info, output_dir)
...
def main():
    ...
    parser.add_argument('--psd-info', type=str, default='../data/image/psd_info.json',
                        help='PSD信息JSON文件路径')
    parser.add_argument('--output-dir', type=str, default='../data/image/layers',
                        help='图层图片输出目录')
    parser.add_argument('--output-file', type=str, default='../src/psd_layout.html',
                        help='输出HTML文件路径')
    args = parser.parse_args()
    
    # 加载PSD信息
    with open(args.psd_info, 'r', encoding='utf-8') as f:
        psd_info = json.load(f)
    
    # 生成HTML
    generate_html(psd_info, args.output_dir, args.output_file)
